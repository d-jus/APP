...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QGridLayout, QLineEdit, QPushButton, QHBoxLayout, QScrollArea, QGroupBox
...
class Kalkulator(QWidget):
    ...
    def interfejs(self):
        ...
        obliczBtn.clicked.connect(self.SEND_TO_MAIN) #
    ...
    def SEND_TO_MAIN(self):
        ...
        try:
            list_ = [float(num.replace(",",".")) for num in list_]
            list_ = [x for x in list_ if x != '']
        except: pass  
        print(list_)
        return('jakiś napis')
        main.loop(list_)
        list_ = [56,	9.1,	0,	0.3,	0.5,	0.6,	20,	20,	30,	1.1,	6.4,	1.6,	0,	2.5,	0,	0]
        #main.loop(list_)
    def set_output(self):
